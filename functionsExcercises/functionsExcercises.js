...
function casino() {
  ...
  do {
    ...
    do {
      ...
      if (!numberPick) {
        // the !numberPick matches to the isNan(numberPick) on line 650
    ...
    alert(
      `The dealer spinnnnnnns the roulette wheel and the ball suddenly comes to a halt. You feel nervous.`
    );
    ...
    if ((isPlaying = confirm("Do you want to play again?"))) {
      ...
      if (money == 0) {
        alert(
          `Unfortunately because you have $${money} left, you cannot continue to play. If you wish to continue, we have an an ATM near the food court.`
        );
        isPlaying = false;
      }
    }
